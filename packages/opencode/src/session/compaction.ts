...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const processCompaction = Effect.fn("SessionCompaction.process")(function* (input: {
      ...
      const cfg = yield* config.get()
      const history = compactionPart && messages.length > 0 && messages[messages.length - 1].info.id === input.parentID ? messages.slice(0, -1) : messages
      ...
      // Use shallow copy instead of structuredClone to avoid doubling memory usage
      const msgs = selected.head.map(m => ({ ...m, parts: [...m.parts] }))
